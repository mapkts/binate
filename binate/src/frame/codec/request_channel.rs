...
/// # Frame Contents
///
/// The request_channel frame is structured as follows:
///
/// ```text
///  0                   1                   2                   3
...
impl RequestChannelFrame {
    ...
    /// - flag `complete` indicates stream completion. If set `on_complete()` or equivalent will be
    /// invoked on Subscriber/Observer.
...
mod tests {
    ...
    #[test]
    fn test_codec() {
        let lease = RequestChannelFrame::new(
            1,
            true,
            true,
            2,
        ...
        assert_eq!(flags, Flags::METADATA | Flags::FOLLOWS | Flags::COMPLETE);

    ...
    fn try_into(self) -> Result<Payload, Self::Error> {
        ...
        self.bufs.into_iter().try_for_each(|mut v| {
            let (m, d) = match Frame::decode(&mut v)? {
                Frame::RequestResponse(v) => v.payload().split(),
                Frame::RequestFnf(v) => v.payload().split(),
                Frame::RequestStream(v) => v.payload().split(),
                Frame::RequestChannel(v) => v.payload().split(),
                Frame::Payload(v) => v.payload().split(),
                _ => (None, None),
            };
            if let Some(b) = m {
                meta.put(b);
